        ...
        <section id="service" className="block service" style={{ backgroundImage: `url(${BgImage})` }}>
            ...
            <Row className="cardflex">
                {data.serviceCategories.map((category) => (
                    <Col key={category.id} xs={5} sm={3} md={3} lg={3}>
                        <Link to={`/services/${category.serviceCategoryName}`}>
                            <Card className="card d-flex flex-column justify-content-center">
                                <Card.Img src={'data:image/png;base64,' + category.categoryImage} variant="top" alt={category} />
                                <Card.Body>
                                    <Card.Text>{category.serviceCategoryName}</Card.Text>
                                </Card.Body>
                            </Card>
                        </Link>
                    </Col>
